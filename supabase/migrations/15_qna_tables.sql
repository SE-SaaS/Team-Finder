...
CREATE POLICY "Questions are viewable by everyone"
  ON qna_questions FOR SELECT USING (true);

CREATE POLICY "Users can insert their own questions"
  ON qna_questions FOR INSERT WITH CHECK (auth.uid() = author_id);

CREATE POLICY "Users can update their own questions"
  ON qna_questions FOR UPDATE USING (auth.uid() = author_id) WITH CHECK (auth.uid() = author_id);

CREATE POLICY "Users can delete their own questions"
  ON qna_questions FOR DELETE USING (auth.uid() = author_id);

CREATE POLICY "Answers are viewable by everyone"
  ON qna_answers FOR SELECT USING (true);

CREATE POLICY "Users can insert their own answers"
  ON qna_answers FOR INSERT WITH CHECK (auth.uid() = author_id);

CREATE POLICY "Users can update their own answers"
  ON qna_answers FOR UPDATE USING (auth.uid() = author_id) WITH CHECK (auth.uid() = author_id);

CREATE POLICY "Users can delete their own answers"
  ON qna_answers FOR DELETE USING (auth.uid() = author_id);

CREATE POLICY "Question votes are viewable by everyone"
  ON qna_question_votes FOR SELECT USING (true);

CREATE POLICY "Users can manage their own question votes"
  ON qna_question_votes FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE POLICY "Answer votes are viewable by everyone"
  ON qna_answer_votes FOR SELECT USING (true);

CREATE POLICY "Users can manage their own answer votes"
  ON qna_answer_votes FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
