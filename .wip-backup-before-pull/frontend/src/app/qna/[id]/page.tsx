      ...
      <main className="max-w-4xl mx-auto px-6 py-6 space-y-8">
        ...
        <section className="border border-[#30363d] rounded-md bg-[#0d1117] p-5">
          ...
          <div className="flex items-center flex-wrap gap-x-3 gap-y-1 text-[11px] text-[#8b949e] mb-4">
            <span>
              by{' '}
              <span className="text-[#c9d1d9]">{question.author_name ?? 'anonymous'}</span>
            </span>
            <span>•</span>
            <span>{new Date(question.created_at).toLocaleString()}</span>
          </div>
          <p className="text-[#c9d1d9] text-sm whitespace-pre-wrap leading-relaxed">
            {question.body}
          </p>
          {question.tags.length > 0 && (
            <div className="mt-4 flex items-center flex-wrap gap-2">
              {question.tags.map((t) => (
                <span
                  key={t}
                  className="text-[11px] px-2 py-0.5 rounded bg-[#1f6feb]/20 text-[#58a6ff]"
                >
                  {t}
                </span>
              ))}
        ...
        <section className="border border-[#30363d] rounded-md bg-[#0d1117] p-5">
          ...
          <form onSubmit={handleSubmitAnswer} className="space-y-3">
            ...
            <p className="text-[11px] text-[#6e7681]">
              5–10000 characters ({answerBody.length}/10000)
            </p>
            {error && (
