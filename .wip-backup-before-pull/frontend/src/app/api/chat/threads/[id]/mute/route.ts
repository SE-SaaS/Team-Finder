...
/**
 * POST /api/chat/threads/[id]/mute
 * Toggles the current user's mute (= archive) state for this thread.
 * Other user is unaffected. Returns the new state.
 */
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  // Read current state (creating the row if it doesn't exist yet)
