...
#  same ai but using firework API 
# implementation

"""
University Career Assistant AI Agent
Built with LangGraph and Fireworks AI (ported from Anthropic/Claude)
"""
...
async def create_university_assistant():
    ...
    if not fireworks_api_key:
        raise ValueError("FIREWORKS_API_KEY must be set in .env file")
    if not database_url:
        raise ValueError("DATABASE_URL must be set in .env file")

    # Swap in any Fireworks-hosted model that supports tool calling.
...
async def run_agent_terminal():
    ...
    print("=" * 60)
    print("University Career Assistant AI")
    print("Powered by Fireworks AI and LangGraph")
    print("=" * 60)
    print("\nType 'exit' or 'quit' to end the conversation.")
    print("Type 'clear' to start a new conversation.\n")
